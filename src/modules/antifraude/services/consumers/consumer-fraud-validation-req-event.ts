...
@Injectable()
export class ApplyFraudValidationConsumer implements OnModuleInit, OnModuleDestroy {


  constructor(
    private readonly updateTransacStatusReqProducer: UpdateTransacStatusReqProducer, // Assuming this is the producer service
    private readonly applyFraudValidationService: ApplyFraudValidationService, // Assuming you have a TransactionRepository to handle DB operations
  ) {}


  private readonly logger = new Logger(ApplyFraudValidationConsumer.name);
  private kafka: Kafka;
  ...
  async onModuleInit() {
    ...
    this.consumer = this.kafka.consumer({ groupId: 'apply-fraud-validation' });
    await this.consumer.connect();
    await this.consumer.subscribe({ topic: TRANSACTION_TOPICS.VALIDATED_REQUEST, fromBeginning: true });
