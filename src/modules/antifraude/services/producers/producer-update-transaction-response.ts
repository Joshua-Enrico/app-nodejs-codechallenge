...
@Injectable()
export class FraudValStatusUpdateResponse implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(FraudValStatusUpdateResponse.name);
  private producer: Producer;

  constructor(
    @Inject('UPDATE_FRAUD_VAL_REQUEST_RESPONSE') private readonly kafkaClient: ClientKafka,
  ) {}
  ...
  async produceTransactionEvent(createTransactionDto: any) {
    ...
    try {
      ...
      const metadata: RecordMetadata[] = await this.producer.send({
        topic: TRANSACTION_TOPICS.VALIDATE_UPDATE_RESPONSE,
